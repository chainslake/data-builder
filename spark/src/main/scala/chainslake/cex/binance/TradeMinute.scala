...
object TradeMinute extends TaskRun {
  ...
  protected def onProcess(spark: SparkSession, outputTable: String, fromBlockTime: Long, toBlockTime: Long, properties: Properties): Unit = {
    ...
    transform(spark, fromBlockTime, toBlockTime, properties)
      .write.format("delta")
  ...
  def transform(spark: SparkSession, fromBlockTime: Long, toBlockTime: Long, properties: Properties): Dataset[CexTradeMinute] = {
    ...
    val inputTable = properties.getProperty("list_input_tables")
    spark.read.table(inputTable).repartition(numberPartition).where(col("quote_asset") === quoteAsset).as[CexExchangeInfo].flatMap(exchange => {
      ...
      val response = httpCall(api + s"&symbol=${exchange.symbol}&startTime=${startTime}&endTime=${endTime}&limit=1500")
